...
function init_profile($data=NULL)
{
	...
	include_once GAME_ROOT.'./include/game/combat/revattr.func.php';
...
function init_battle_rev($pa,$pd,$ismeet=0)
{
	...
	include_once GAME_ROOT.'./include/gamectl/init.func.php';
	//初始化头像显示
...
// ===== 以下函数已迁移至 include/game/render.func.php =====
// The following functions moved to include/game/render.func.php:
//   is_rich_template_enabled(), init_bgm(), render_bgm_player(),
//   init_mapdata(), init_clubskillsdata()
...
function check_skilllasttimes(&$data)
{
	...
	{
		//include_once GAME_ROOT.'./include/game/club/revclubskills.func.php';
...
//获取安全地图范围（已废弃，请使用 deatharea.func.php 中的 get_safe_areas() 或 get_safe_areas_ex()）
function get_safe_plslist($mode=1)
{
	global $danger_areas;
	$r = get_safe_areas();
	if($mode) $r = array_diff($r, $danger_areas);
	return $r;
}
