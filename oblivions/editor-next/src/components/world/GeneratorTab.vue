...
//
// GeneratorTab：生成器子 Tab（迁移自 GeneratorsView 主体，对齐 P1 执行案 §4.10.3）
//
// 设计意图：
//   - 复用现有 5 个生成器卡片网格 + 双入口按钮
//   - GeneratorModal 由本组件挂载（与原 GeneratorsView 一致）
//   - 生成器调用 projectStore.addGeneratedRegion 写入 graph-store
//     （通过 GeneratorModal 内部包装为 Command 进入 historyStore）
//
// 边界：
//   - GeneratorTab 是 WorldView 的子组件，不是顶层 view
//   - 子 Tab 切换时若 GeneratorModal 打开，模态保持（uiStore 全局状态）
//   - 生成结果作为 Change Set 的批量变更（由 projectStore.addGeneratedRegion
//     内部调用 graph.applyNodeBatch 实现，避免逐个响应式更新）
