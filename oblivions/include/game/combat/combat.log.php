...
// ================================================================
// 新战斗系统 — battlelog v3 领域事件构造器
//
// 职责：复用 BattleLogCollector，提供新系统专用便捷方法。
//   - 输出携带权威回合坐标的 battlelog.v3 typed events
...
BattleLogCollector::registerPhase('battlelog_v3', false);

// 返回当前 battlelog 版本标识（v2）
function combat_log_v3_mode(): string {
    return 'v3';
}

// 是否启用 v2 日志格式（始终 true，v1 格式已废弃）
function combat_log_v3_enabled(): bool {
    return true;
}

// 生成全局唯一事件 UID：格式 "blv2-{request_uid}-{event_type}-{seq}"
function combat_log_v3_next_event_uid(string $event_type): string {
    static $seq = 0;
    $seq++;
    $request_uid = (string)($GLOBALS['obl_request_uid'] ?? 'request-unknown');
    return 'blv3-' . $request_uid . '-' . $event_type . '-' . $seq;
}

// 生成 action 级唯一标识：格式 "{operation_key}-q{qid}-p{pid}-a{seq}-{act_id}"
// 用于关联同一 action 链下的所有事件
function combat_log_v3_make_action_uid(array $actor_data, string $act_id, int $seq): string {
...
function combat_log_v3_combatant_snapshot($data): ?array {
...
function combat_log_v3_target_ref(CombatContext $ctx, array $target): array {
...
function combat_log_v3_actor_target_ref(CombatContext $ctx): array {
    return [
        'kind' => 'self',
        'pid' => (int)($ctx->actor_data['pid'] ?? 0),
        'snapshot' => combat_log_v3_combatant_snapshot($ctx->actor_data),
...
function combat_log_v3_turn_opened(
    $log,
    array $turn,
    array $actor_data,
    string $controller,
    ...
    if (!$log || !combat_log_v3_enabled()) return;
    $coords = battle_turn_coordinates($turn);
    combat_log_v3_emit($log, array_merge($coords, [
        'event_type' => 'turn_opened',
        'event_uid' => 'battle:' . $coords['qid'] . ':turn:' . $coords['turn_seq'] . ':opened',
        'actor_pid' => (int)($actor_data['pid'] ?? 0),
        'payload' => [
            'actor' => combat_log_v3_combatant_snapshot($actor_data),
            'controller' => $controller === 'player' ? 'player' : 'system',
...
function combat_log_v3_emit($log, array $event): void {
    if (!$log || !combat_log_v3_enabled()) return;
    ...
    $event_qid = $event['qid'] ?? ($payload['qid'] ?? null);
    $coords = array(
        'qid' => $event_qid,
        'round_num' => $event['round_num'] ?? null,
        'turn_seq' => $event['turn_seq'] ?? null,
        'turn_key' => $event['turn_key'] ?? null,
    );
    $active = function_exists('battle_turn_get_event_context') ? battle_turn_get_event_context() : null;
    if (is_array($active)
        && ((int)$event_qid <= 0 || (int)$event_qid === (int)($active['qid'] ?? 0))) {
        foreach ($coords as $field => $value) {
            if ($value === null) $coords[$field] = $active[$field] ?? null;
        }
    }

    $log->setPhase('battlelog_v3');
    $log->emit([
        'schema'     => 'battlelog.v3',
        'event_type' => $event_type,
        'channel'    => $channel,
        'event_uid'  => (string)($event['event_uid'] ?? combat_log_v3_next_event_uid($event_type)),
        'action_uid' => $event['action_uid'] ?? ($payload['action_uid'] ?? null),
        'effect_uid' => $event['effect_uid'] ?? ($payload['effect_uid'] ?? null),
        'payload'    => $payload,
        'qid'        => $coords['qid'] ?? ($payload['qid'] ?? null),
        'round_num'  => $coords['round_num'],
        'turn_seq'   => $coords['turn_seq'],
        'turn_key'   => $coords['turn_key'],
...
function combat_log_v3_action_start(CombatContext $ctx): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    if ($ctx->event_action_started) return;
    $ctx->event_action_started = true;

    $targets = [];
    foreach ($ctx->targets as $target) {
        if (!empty($target['skip'])) continue;
        $targets[] = combat_log_v3_target_ref($ctx, $target);
    }

    combat_log_v3_emit($ctx->log, [
        ...
        'payload' => [
            ...
            'action_uid' => $ctx->action_uid,
            'action_id' => $ctx->act_id,
            'actor' => combat_log_v3_combatant_snapshot($ctx->actor_data),
...
function combat_log_v3_action_delivery(CombatContext $ctx, string $delivery_type, array $resolved_aim): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    combat_log_v3_emit($ctx->log, [
...
function combat_log_v3_combatant_joined(CombatContext $ctx, array $target_data, int $myorder): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    combat_log_v3_emit($ctx->log, [
        ...
        'payload' => [
            'qid' => (int)($ctx->actor_data['bid'] ?? 0),
            'combatant' => combat_log_v3_combatant_snapshot($target_data),
...
function combat_log_v3_effect_applied(CombatContext $ctx, string $effect_type, array $payload): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    $effect_uid = $ctx->action_uid . '-e' . (count($ctx->event_effect_uids) + 1);
    $ctx->event_effect_uids[] = $effect_uid;
    if (!isset($ctx->event_effect_uids_by_type[$effect_type])) {
        $ctx->event_effect_uids_by_type[$effect_type] = [];
    }
    $ctx->event_effect_uids_by_type[$effect_type][] = $effect_uid;
    ...
    $event_payload = array_merge([
        ...
        'source' => combat_log_v3_combatant_snapshot($ctx->actor_data),
    ], $payload);
...
function combat_log_v3_action_end(CombatContext $ctx): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    if ($ctx->event_action_ended) return;
    $ctx->event_action_ended = true;
...
function combat_log_v3_action_failed($log, array $actor_data, string $act_id, string $reason, array $extra = [], ?string $action_uid = null): void {
    if (!$log || !combat_log_v3_enabled()) return;
    combat_log_v3_emit($log, [
...
function combat_log_v3_action_failed_from_context(CombatContext $ctx, string $reason, array $extra = []): void {
    if (!$ctx->log || !combat_log_v3_enabled()) return;
    ...
    if ($ctx->event_action_started && !$ctx->event_action_ended) {
        $ctx->success = false;
        combat_log_v3_action_end($ctx);
    }

    combat_log_v3_action_failed($ctx->log, $ctx->actor_data, $ctx->act_id, $reason, $extra, $ctx->action_uid);
}

// 发射 combatant_cleared 事件：记录 combatant 因为 dead/escaped 等原因离开战场
function combat_log_v3_combatant_cleared(
    $log,
    ...
    combat_log_v3_emit($log, [
        ...
        'payload' => array_merge([
            'combatant' => combat_log_v3_combatant_snapshot($combatant_data),
            'reason' => $contract_reason,
...
function combat_log_v3_battle_end($log, string $reason, int $winner_pid = 0, array $survivors = [], int $qid = 0): void {
    if (!$log || !combat_log_v3_enabled()) return;
    combat_log_v3_emit($log, [
