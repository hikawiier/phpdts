<?php
/**
 * @module F 物品系统
 */
if (!defined('IN_GAME')) { exit('Access Denied'); }

// ================================================================
// Oblivions 道具模板表
// 唯一 ID 索引 → 道具模板属性
// 其他文件引用道具时只需引用 ID，运行时从此表读取模板实例化
// 字段与现有物品系统对齐：itm/itmk/itme/itms/itmsk/itmpara
//
// 字段说明：
// - itm/desc：deprecated，文案已迁移至前端 vex-vue/src/data/item-locale.ts
// - tags：Tag ID 数组（性质描述 Tag + 系统钩子 Tag，共存于同一字段）
// - use_effect：使用效果函数名（非空时 tags 必须含 tag_usable，由数据校验保证）
// - tool_level：工具等级（仅工作台/工具类道具有效），0=无等级要求；高级兼容低级
//
// 数据校验规则（见《道具-系统钩子Tag设计案.md》§6.3）：
// - itmk ∈ {WP,WK,WG,WD,WF,WC,AR,AH,AF,AA} ⟹ tags 含 tag_equippable
// - use_effect 非空 ⟺ tags 含 tag_usable
// - itmk ∈ {MT,HH,HS,DX} ⟹ tags 不含 tag_equippable
// ================================================================

return [
    // ─── 武器 ───────────────────────────────────────────

    'rusty_pipe' => [
        'itm'      => '生锈的水管',
        'itmk'     => 'WP',
        'itme'     => 8,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '一根锈迹斑斑的铁管，握在手里沉甸甸的。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'scrap_blade' => [
        'itm'      => '废铁刀',
        'itmk'     => 'WK',
        'itme'     => 12,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '用废铁片磨出的粗糙刀刃，勉强能割开东西。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'nail_gun' => [
        'itm'      => '钉枪',
        'itmk'     => 'WG',
        'itme'     => 10,
        'itms'     => 8,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '工地上常见的气动钉枪，近距离威力不小。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'pipe_bomb' => [
        'itm'      => '管状炸弹',
        'itmk'     => 'WD',
        'itme'     => 25,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '用铁管和火药自制的简易爆炸物，小心别炸到自己。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'swamp_spear' => [
        'itm'      => '沼泽长矛',
        'itmk'     => 'WK',
        'itme'     => 18,
        'itms'     => 25,
        'itmsk'    => 'p',
        'itmpara'  => '',
        'desc'     => '用沼泽硬木和骨片制成的长矛，尖端涂有毒素。',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'throwing_spear' => [
        'itm'      => '投矛',
        'itmk'     => 'WC',
        'itme'     => 12,
        'itms'     => 10,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '适合投掷的短矛，能在中距离刺穿敌人。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_weapon_throwing', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'ancient_core_blade' => [
        'itm'      => '核心刃',
        'itmk'     => 'WF',
        'itme'     => 30,
        'itms'     => 15,
        'itmsk'    => 'f',
        'itmpara'  => '',
        'desc'     => '嵌入古代核心的武器，刃身散发微弱的光辉。',
        'tier'     => 'epic',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 防具 ───────────────────────────────────────────

    'scrap_vest' => [
        'itm'      => '废铁背心',
        'itmk'     => 'AR',
        'itme'     => 6,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '用铁皮和铁丝拼凑的简易护甲，聊胜于无。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'swamp_cloak' => [
        'itm'      => '沼泽斗篷',
        'itmk'     => 'AR',
        'itme'     => 10,
        'itms'     => 25,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '用沼泽藤蔓编织的斗篷，能抵御部分攻击。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'rust_circlet' => [
        'itm'      => '锈蚀头环',
        'itmk'     => 'AH',
        'itme'     => 4,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '锈蚀的金属头环，提供基本的头部防护。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'bone_amulet' => [
        'itm'      => '骨制护符',
        'itmk'     => 'AA',
        'itme'     => 5,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '用不明骨骼雕刻的护符，散发着诡异的气息。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 消耗品 ─────────────────────────────────────────
    // 现有消耗品（bread/mineral_water/health_potion 等）已接入 use_effect 框架，
    // 具体效果函数由归属系统（食物经验/健康系统）注册。注册前使用会 emit use_item.effect_not_registered。

    'bread' => [
        'itm'      => '面包',
        'itmk'     => 'HH',
        'itme'     => 120,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '朴素的干面包，能快速补充生命。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 15,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'mineral_water' => [
        'itm'      => '矿泉水',
        'itmk'     => 'HS',
        'itme'     => 140,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '未开封的瓶装水，能恢复体力。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 15,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],

    'scrap_metal' => [
        'itm'      => '废铁片',
        'itmk'     => 'MT',
        'itme'     => 5,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '锈迹斑斑的金属碎片，也许能派上用场。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'rusty_gear' => [
        'itm'      => '生锈齿轮',
        'itmk'     => 'MT',
        'itme'     => 3,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '废弃机械中拆下的齿轮，也许能合成什么。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'health_potion' => [
        'itm'      => '生命药剂',
        'itmk'     => 'HH',
        'itme'     => 80,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '注入了回复魔力的红色液体，饮用后恢复生命。',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'stamina_potion' => [
        'itm'      => '体力药剂',
        'itmk'     => 'HS',
        'itme'     => 50,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '淡绿色的液体，饮用后恢复体力。',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],
    'supply_pack' => [
        'itm'      => '补给包',
        'itmk'     => 'HH',
        'itme'     => 30,
        'itms'     => 3,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '简易的急救补给，可多次使用。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'antidote' => [
        'itm'      => '解毒剂',
        'itmk'     => 'DX',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '能中和常见毒素的药剂，沼泽中不可或缺。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'cure_bs',
        'tool_level' => 0,
    ],
    'swamp_herb' => [
        'itm'      => '沼泽草药',
        'itmk'     => 'HH',
        'itme'     => 20,
        'itms'     => 1,
        'itmsk'    => 'p',
        'itmpara'  => '',
        'desc'     => '沼泽中生长的草药，能恢复少量生命但有轻微毒性。',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 8,
        'tags'     => ['tag_raw_food'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'ancient_core' => [
        'itm'      => '古代核心',
        'itmk'     => 'MT',
        'itme'     => 50,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '散发微光的神秘核心，蕴含未知的能量。',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 工具 ───────────────────────────────────────────

    'rope_coil' => [
        'itm'      => '绳索',
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 3,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '一卷结实的绳索，在沼泽地带可能派上用场。',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'compass' => [
        'itm'      => '指南针',
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => '∞',
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '老旧但还能用的指南针，在迷雾中辨别方向。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'lockpick' => [
        'itm'      => '开锁器',
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 5,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '简易的开锁工具，也许能打开某些宝箱。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_lockpick', 'tag_tool', 'tag_poi_interactive'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 特殊 ───────────────────────────────────────────

    'element_pocket' => [
        'itm'      => '元素口袋',
        'itmk'     => 'SP',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '能收纳元素之力的神秘口袋，元素大师专属道具。',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'mystery_box' => [
        'itm'      => '神秘礼盒',
        'itmk'     => 'SP',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'desc'     => '包装精美的礼盒，打开后才知道里面是什么。',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_usable'],
        'use_effect' => 'open_gift_box',
        'tool_level' => 0,
    ],

    // ================================================================
    // ─── 道具系统扩展（主设计案 v6.2 §3.2）──────────────────────────
    // ================================================================

    // ─── 基础素材（性质描述 Tag）──────────────────────────

    'cloth' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'blade_shard' => [
        'itmk'     => 'MT',
        'itme'     => 2,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'circuit_board' => [
        'itmk'     => 'MT',
        'itme'     => 5,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'crushed_can' => [
        'itmk'     => 'MT',
        'itme'     => 2,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'scrap_wire' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'cabin_sponge' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 食物（可使用，use_effect 由归属系统注册）──────────

    'rabbit_meat_raw' => [
        'itmk'     => 'HH',
        'itme'     => 20,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_raw_food', 'tag_usable'],
        'use_effect' => 'gain_resistance',
        'tool_level' => 0,
    ],
    'roasted_rabbit' => [
        'itmk'     => 'HH',
        'itme'     => 80,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],
    'bandage' => [
        'itmk'     => 'HH',
        'itme'     => 0,
        'itms'     => 3,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable'],
        'use_effect' => 'cure_bs',
        'tool_level' => 0,
    ],
    'simple_stew' => [
        'itmk'     => 'HH',
        'itme'     => 150,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],

    // ─── 装备产物（可装备）──────────────────────────────

    'thick_shoes' => [
        'itmk'     => 'AF',
        'itme'     => 3,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'blade_wrapped' => [
        'itmk'     => 'WK',
        'itme'     => 10,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_sharp', 'tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 工具/工作台道具（性质描述 Tag + tool_level）──────

    'frying_pan' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_cooking'],
        'use_effect' => '',
        'tool_level' => 1,
    ],
    'precision_stove' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 50,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_tool_cooking'],
        'use_effect' => '',
        'tool_level' => 2,
    ],
    'forge_t1' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => '∞',
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_forge'],
        'use_effect' => '',
        'tool_level' => 1,
    ],
    'stove_t1' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => '∞',
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_cooking'],
        'use_effect' => '',
        'tool_level' => 1,
    ],
    'stove_t2' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => '∞',
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_tool_cooking'],
        'use_effect' => '',
        'tool_level' => 2,
    ],

    // ================================================================
    // ─── 道具素材扩展（附录A：搜索建筑物与掉落机制重构）──────────────
    // 来源：oblivions/docs/原始方案/items.json（外部参考，已剔除容器/剧情/GUI）
    // 字段约定：itm/desc 已迁移至前端 item-locale.ts，本节遵循扩展区约定不写
    // 详见：oblivions/docs/搜索建筑物与掉落机制重构-附录A-道具素材扩展.md
    // ================================================================

    // ─── 近战武器（WP 钝器 / WK 刃具）──────────────────────────

    'glass_knife' => [
        'itmk'     => 'WK',
        'itme'     => 6,
        'itms'     => 8,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'wooden_club' => [
        'itmk'     => 'WP',
        'itme'     => 10,
        'itms'     => 25,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'kitchen_knife' => [
        'itmk'     => 'WK',
        'itme'     => 11,
        'itms'     => 18,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'crowbar' => [
        'itmk'     => 'WP',
        'itme'     => 14,
        'itms'     => 40,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight', 'tag_tool_crowbar', 'tag_poi_interactive'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'sharpened_spear' => [
        'itmk'     => 'WK',
        'itme'     => 16,
        'itms'     => 30,
        'itmsk'    => 'p',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'broad_spear' => [
        'itmk'     => 'WK',
        'itme'     => 22,
        'itms'     => 35,
        'itmsk'    => 'p',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'multitool_knife' => [
        'itmk'     => 'WK',
        'itme'     => 14,
        'itms'     => 50,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_sharp', 'tag_tool'],
        'use_effect' => '',
        'tool_level' => 1,
    ],
    'wooden_splint' => [
        'itmk'     => 'WP',
        'itme'     => 5,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 远程武器（WG 枪械 / 弓）──────────────────────────────

    'self_bow' => [
        'itmk'     => 'WG',
        'itme'     => 15,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'compound_bow' => [
        'itmk'     => 'WG',
        'itme'     => 25,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'sling' => [
        'itmk'     => 'WG',
        'itme'     => 6,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'revolver' => [
        'itmk'     => 'WG',
        'itme'     => 35,
        'itms'     => 50,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'hunting_rifle' => [
        'itmk'     => 'WG',
        'itme'     => 50,
        'itms'     => 60,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'epic',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 投掷武器（WC）──────────────────────────────────────

    'stone_pebble' => [
        'itmk'     => 'WC',
        'itme'     => 3,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_equippable', 'tag_weapon_throwing'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'rock' => [
        'itmk'     => 'WC',
        'itme'     => 8,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_equippable', 'tag_weapon_throwing', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'hunting_arrow' => [
        'itmk'     => 'WC',
        'itme'     => 10,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_equippable', 'tag_weapon_throwing', 'tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 护甲（AR）──────────────────────────────────────────

    'tactical_vest' => [
        'itmk'     => 'AR',
        'itme'     => 18,
        'itms'     => 50,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'beast_hide_coat' => [
        'itmk'     => 'AR',
        'itme'     => 12,
        'itms'     => 40,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'scrap_metal_raincoat' => [
        'itmk'     => 'AR',
        'itme'     => 8,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 头具（AH）──────────────────────────────────────────

    'tactical_helmet' => [
        'itmk'     => 'AH',
        'itme'     => 12,
        'itms'     => 40,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'gas_mask' => [
        'itmk'     => 'AH',
        'itme'     => 8,
        'itms'     => 35,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'assassin_hood' => [
        'itmk'     => 'AH',
        'itme'     => 4,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'night_vision_goggles' => [
        'itmk'     => 'AH',
        'itme'     => 6,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'epic',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_tool_vision'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 足具（AF）──────────────────────────────────────────

    'cloth_shoes' => [
        'itmk'     => 'AF',
        'itme'     => 2,
        'itms'     => 15,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'sneakers' => [
        'itmk'     => 'AF',
        'itme'     => 3,
        'itms'     => 25,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'combat_boots' => [
        'itmk'     => 'AF',
        'itme'     => 5,
        'itms'     => 35,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 配件（AA）──────────────────────────────────────────

    'bronze_amulet' => [
        'itmk'     => 'AA',
        'itme'     => 4,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'binoculars' => [
        'itmk'     => 'AA',
        'itme'     => 2,
        'itms'     => 25,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_tool_vision'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'rifle_scope' => [
        'itmk'     => 'AA',
        'itme'     => 3,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_equippable', 'tag_tool_vision'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'tactical_gloves' => [
        'itmk'     => 'AA',
        'itme'     => 3,
        'itms'     => 25,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_equippable'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 材料（MT）──────────────────────────────────────────

    'dirty_rag' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'clean_cloth' => [
        'itmk'     => 'MT',
        'itme'     => 2,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'tarpaulin' => [
        'itmk'     => 'MT',
        'itme'     => 4,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'tree_branch' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'aa_battery' => [
        'itmk'     => 'MT',
        'itme'     => 2,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_conductor'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'laptop_battery' => [
        'itmk'     => 'MT',
        'itme'     => 8,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_conductor'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'glass_shard' => [
        'itmk'     => 'MT',
        'itme'     => 2,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_sharp'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'plastic_shard' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'small_parts' => [
        'itmk'     => 'MT',
        'itme'     => 3,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'wooden_stock' => [
        'itmk'     => 'MT',
        'itme'     => 5,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'rifle_barrel' => [
        'itmk'     => 'MT',
        'itme'     => 15,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'rare',
        'stack'    => false,
        'tags'     => ['tag_heavy_weight'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'animal_pelt' => [
        'itmk'     => 'MT',
        'itme'     => 3,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => [],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'old_newspaper' => [
        'itmk'     => 'MT',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_combustible'],
        'use_effect' => '',
        'tool_level' => 0,
    ],

    // ─── 恢复品（HH 食物）──────────────────────────────────────

    'soda_crackers' => [
        'itmk'     => 'HH',
        'itme'     => 60,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'wild_mushroom' => [
        'itmk'     => 'HH',
        'itme'     => 30,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable', 'tag_raw_food'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],
    'raw_meat_chunk' => [
        'itmk'     => 'HH',
        'itme'     => 25,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable', 'tag_raw_food', 'tag_perishable'],
        'use_effect' => 'gain_resistance',
        'tool_level' => 0,
    ],
    'cooked_meat_chunk' => [
        'itmk'     => 'HH',
        'itme'     => 100,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'cured_meat' => [
        'itmk'     => 'HH',
        'itme'     => 120,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'berries' => [
        'itmk'     => 'HH',
        'itme'     => 20,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 8,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],
    'concentrated_soup' => [
        'itmk'     => 'HH',
        'itme'     => 180,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],

    // ─── 状态品（HS 饮料）──────────────────────────────────────

    'cola' => [
        'itmk'     => 'HS',
        'itme'     => 60,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],
    'whiskey' => [
        'itmk'     => 'HS',
        'itme'     => 40,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'cure_bs',
        'tool_level' => 0,
    ],
    'herbal_tea' => [
        'itmk'     => 'HS',
        'itme'     => 80,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 5,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_sp',
        'tool_level' => 0,
    ],

    // ─── 药物（DX）──────────────────────────────────────────

    'orange_pill' => [
        'itmk'     => 'DX',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'painkiller' => [
        'itmk'     => 'DX',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_usable'],
        'use_effect' => 'cure_bs',
        'tool_level' => 0,
    ],
    'white_pill' => [
        'itmk'     => 'DX',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 10,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],
    'nano_medkit' => [
        'itmk'     => 'DX',
        'itme'     => 1,
        'itms'     => 1,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'epic',
        'stack'    => false,
        'tags'     => ['tag_usable'],
        'use_effect' => 'restore_hp',
        'tool_level' => 0,
    ],

    // ─── 工具（TK）──────────────────────────────────────────

    'flashlight' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 50,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_light'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'lighter' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_igniter', 'tag_poi_interactive'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'lockpick_set' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 10,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool_lockpick', 'tag_tool', 'tag_poi_interactive'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'torch_unlit' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 20,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'common',
        'stack'    => true,
        'stack_limit' => 3,
        'tags'     => ['tag_tool_light', 'tag_combustible', 'tag_poi_interactive'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
    'water_tester' => [
        'itmk'     => 'TK',
        'itme'     => 1,
        'itms'     => 30,
        'itmsk'    => '',
        'itmpara'  => '',
        'tier'     => 'uncommon',
        'stack'    => false,
        'tags'     => ['tag_tool'],
        'use_effect' => '',
        'tool_level' => 0,
    ],
];
